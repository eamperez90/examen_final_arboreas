...
public class Main {
    ...
    private static Nodo insertRec(Nodo nodo, int valor) {
        if (nodo == null) return new Nodo(valor);
        if (valor < nodo.valor)
            nodo.izquierdo = insertRec(nodo.izquierdo, valor);
        else if (valor > nodo.valor)
            nodo.derecho = insertRec(nodo.derecho, valor);
        return nodo;
    }
    ...
    //  SEARCH
    // ═════════════════════════════════════════════════════════════════════════
    public static boolean search(int valor) {
        return searchRec(raiz, valor);
    }

    private static boolean searchRec(Nodo nodo, int valor) {
        if (nodo == null)        return false;
        if (valor == nodo.valor) return true;
        if (valor < nodo.valor)  return searchRec(nodo.izquierdo, valor);
    ...
    private static Nodo deleteRec(Nodo nodo, int valor) {
        if (nodo == null) {
            System.out.println(RED + "  [!] " + valor + " no existe en el arbol." + RESET);
            return null;
        }
        if (valor < nodo.valor) {
            ...
            if (nodo.izquierdo == null && nodo.derecho == null) {
                System.out.println(GREEN + "  [DELETE] Caso 1: nodo hoja " + valor + " eliminado." + RESET);
                return null;
            }
            // Caso 2a: solo hijo derecho
            if (nodo.izquierdo == null) {
                System.out.println(GREEN + "  [DELETE] Caso 2: " + valor + " reemplazado por hijo derecho." + RESET);
                return nodo.derecho;
            }
            // Caso 2b: solo hijo izquierdo
            if (nodo.derecho == null) {
                System.out.println(GREEN + "  [DELETE] Caso 2: " + valor + " reemplazado por hijo izquierdo." + RESET);
                return nodo.izquierdo;
            }
            // Caso 3: dos hijos — sucesor in-order
            Nodo sucesor = minimo(nodo.derecho);
            System.out.println(GREEN + "  [DELETE] Caso 3: " + valor + " -> sucesor in-order = " + sucesor.valor + RESET);
    ...
    private static Nodo minimo(Nodo nodo) {
    ...
    static void imprimirArbol() {
        System.out.println("  Estructura (raiz izquierda -> hojas derecha):");
        imprimirRec(raiz, "", true);
    }
    private static void imprimirRec(Nodo n, String pre, boolean ultimo) {
        if (n != null) {
    ...
    static void eliminar() {
        ...
        if (estaVacio()) { System.out.println(RED + "  El arbol esta vacio." + RESET); return; }
        System.out.println("  Arbol actual:"); recorridoInOrder(); System.out.println();
    ...
    static void recorridos() {
        ...
        recorridoInOrder();
        System.out.println();
        System.out.println(CYAN + "  PreOrder  = raiz primero (copiar arbol):" + RESET);
        recorridoPreOrder();
        System.out.println();
        System.out.println(CYAN + "  PostOrder = raiz al final (liberar memoria):" + RESET);
        recorridoPostOrder();
    }
    ...
    static void menu() {
        ...
        System.out.println("  " + PURPLE + "[4]" + RESET + " Ver recorridos");
